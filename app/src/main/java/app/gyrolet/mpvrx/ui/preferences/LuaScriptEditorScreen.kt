...
data class LuaScriptEditorScreen(
  ...
  override fun Content() {
    ...
    fun saveScript() {
      if (fileName.isBlank()) {
        Toast.makeText(context, context.getString(R.string.editor_file_name_missing_toast), Toast.LENGTH_SHORT).show()
        return
      }
      ...
      scope.launch(Dispatchers.IO) {
        try {
          if (mpvConfStorageLocation.isBlank()) {
            withContext(Dispatchers.Main) {
              Toast.makeText(context, context.getString(R.string.editor_no_storage_toast), Toast.LENGTH_LONG).show()
            }
            return@launch
          }
          ...
          val uri = scriptFile?.uri ?: run {
            withContext(Dispatchers.Main) {
              Toast.makeText(context, context.getString(R.string.editor_create_failed_toast), Toast.LENGTH_LONG).show()
            }
            return@launch
          }
          ...
          context.contentResolver.openOutputStream(uri, "wt")?.use { out ->
            ...
            withContext(Dispatchers.Main) {
              Toast.makeText(context, context.getString(R.string.editor_output_stream_failed_toast), Toast.LENGTH_LONG).show()
            }
            return@launch
          }
          ...
          withContext(Dispatchers.Main) {
            hasUnsavedChanges = false
            Toast.makeText(context, context.getString(R.string.editor_saved_toast, finalFileName), Toast.LENGTH_SHORT).show()
            backStack.popSafely()
          }
        } catch (e: Exception) {
          withContext(Dispatchers.Main) {
            Toast.makeText(context, context.getString(R.string.editor_save_failed_toast, e.message ?: "Unknown error"), Toast.LENGTH_LONG).show()
          }
        }
      }
    }
    ...
    fun shareScript() {
      if (isNewScript) {
        Toast.makeText(context, context.getString(R.string.editor_save_before_share_toast), Toast.LENGTH_SHORT).show()
        return
      }
      ...
      scope.launch(Dispatchers.IO) {
        try {
          ...
          withContext(Dispatchers.Main) {
            Toast.makeText(
              context,
              context.getString(R.string.editor_share_failed_toast, e.message ?: "Unknown error"),
              Toast.LENGTH_LONG
            ).show()
          }
        }
      ...
      scope.launch(Dispatchers.IO) {
        try {
          ...
          if (tree != null && tree.exists()) {
              ...
              if (scriptFile != null && scriptFile.exists()) {
                ...
                if (deleted) {
                  ...
                  withContext(Dispatchers.Main) {
                    Toast.makeText(context, context.getString(R.string.editor_deleted_toast, scriptName), Toast.LENGTH_SHORT).show()
                    backStack.popSafely()
                  }
                }
              }
          ...
          withContext(Dispatchers.Main) {
            Toast.makeText(
              context,
              context.getString(R.string.editor_delete_failed_toast, e.message ?: "Unknown error"),
              Toast.LENGTH_LONG
            ).show()
          }
        }
        ...
        title = {
          Column {
            androidx.compose.foundation.text.BasicTextField(
              ...
              decorationBox = { innerTextField ->
                Box {
                  if (fileName.isEmpty()) {
                    Text(
                      text = stringResource(R.string.editor_script_name_placeholder),
            ...
            Row(
              ...
              ScriptExtensionChip(
                label = stringResource(R.string.editor_lang_lua),
              ...
              ScriptExtensionChip(
                label = stringResource(R.string.editor_lang_js),
    ...
    if (showDeleteDialog) {
      ConfirmDialog(
        title = stringResource(R.string.editor_delete_dialog_title),
        subtitle = stringResource(R.string.editor_delete_dialog_subtitle, scriptName ?: fileName),
        onConfirm = {
          deleteScript()
